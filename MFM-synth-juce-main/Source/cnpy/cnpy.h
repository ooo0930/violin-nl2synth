...
namespace {
    juce::String vector_to_string(std::vector<unsigned long long> v) {
        juce::String res = juce::String("");
        for (auto i : v) {
            res += "," + juce::String(i);
        }
        return res;
    ...
    juce::String vector_to_string_(std::vector<float> v) {
        juce::String res = juce::String("");
        int i = 0;
        for (auto entry : v) {
            res += "," + juce::String(entry);
            i += 1;
            if (i > 10) {
                return res;
            }
...
namespace cnpy {
    ...
    void parse_npy_header(juce::InputStream& fp,size_t& word_size, std::vector<size_t>& shape, bool& fortran_order);
    void parse_npy_header(unsigned char* buffer,size_t& word_size, std::vector<size_t>& shape, bool& fortran_order);
    void parse_zip_footer(FILE* fp, uint16_t& nrecs, size_t& global_header_size, size_t& global_header_offset);
    NpyArray npy_load(std::string fname);
	NpyArray load_the_npy_file(juce::InputStream& fp);



    NpyArray read_npz(std::string path, std::string key);
